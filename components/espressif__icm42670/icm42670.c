...
icm42670_handle_t icm42670_create(i2c_master_bus_handle_t i2c_bus_handle, const uint8_t dev_addr)
{
    icm42670_dev_t *sensor = (icm42670_dev_t *) heap_caps_calloc(1, sizeof(icm42670_dev_t), MALLOC_CAP_DEFAULT);
    sensor->bus = i2c_bus_handle;
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(i2c_bus_handle, &dev_conf, &i2c_master_dev_handle));
...
static esp_err_t icm42670_write(icm42670_handle_t sensor, const uint8_t reg_start_addr, const uint8_t *data_buf, const uint8_t data_len)
{
    ...
    size_t write_size = data_len + 1;
    uint8_t *write_buffer = malloc(write_size);
    if (write_buffer == NULL) {
        return ESP_ERR_NO_MEM;
    }

    write_buffer[0] = reg_start_addr;
    memcpy(write_buffer + 1, data_buf, data_len);

    ret = i2c_master_transmit(i2c_master_dev_handle, write_buffer, write_size, -1);
...
static esp_err_t icm42670_read(icm42670_handle_t sensor, const uint8_t reg_start_addr, uint8_t *data_buf, const uint8_t data_len)
{
    ...
    ret = i2c_master_transmit_receive(i2c_master_dev_handle, reg_buff, sizeof(reg_buff), data_buf, data_len, -1);
    return ret;
}
